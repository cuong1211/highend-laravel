<script>
    ...
    var dt = $("#kt_customers_table").DataTable({
        ...
        ajax: {
            ...
            url: "{{ route('color.show', ['product' => "${product_id}", 'color' => 'get-list']) }}",
            type: 'GET'
        },
        columns: [{
                data: 'null',
    ...
    $(document).on('click', '.btn-edit', function(e) {
        ...
        e.preventDefault();
        form_reset();
        $('.modal-title').text('Sửa thong tin màu');
        ...
        if (data.image.length > 0) {
            for (let i = 0; i < data.image.length; i++) {
                var imageUrl = '{{ route('image', '') }}' + '/' + data.image[i].image;
                $('#imagePreview').append(
                    '<img src="' + imageUrl +
                    '" style="width: 80px; height: 80px; margin-top: 20px; margin-right: 5px; margin-bottom: 20px; border-radius: 5px; border: 1px solid rgb(221, 221, 221);">'
                );
            }
        }
    });
    ...
    $('#kt_modal_add_customer_form').on('submit', function(e) {
        ...
        let type = 'POST',
            url = "{{ route('color.store', ['product' => "${product}"]) }}",
